...
function renderConfigForm(cfg) {
  ...
  var groups = [
    ...
    {title:'桥接设置', fields:[
      ...
      {key:'mention_awake_timeout', label:'@触发免@时长(秒)', type:'number', ph:'180'},
    ...
    def do_POST(self):
        if self.path == "/start":
            ...
            try:
                ...
                # 运行时同步机器人昵称（@ 匹配即时生效，无需重启）
                if "bot_nicknames" in new_cfg:
                    config.BOT_NICKNAMES = new_cfg["bot_nicknames"]
                    log.info(f"[Web] 机器人昵称已更新为: {config.BOT_NICKNAMES}")
