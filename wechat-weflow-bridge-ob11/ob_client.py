...
import state
import config
from ob_protocol import _handle_ob_api, make_lifecycle_event
    ...
    while state.running:
        try:
            ...
            async with websockets.connect(
                ...
                try:
                    await ws.send(json.dumps(make_lifecycle_event("connect"), ensure_ascii=False))
                    log.info("[OB11] 已发送 lifecycle.connect")
                except Exception as e:
                    log.warning(f"[OB11] 发送 lifecycle 失败: {e}")
